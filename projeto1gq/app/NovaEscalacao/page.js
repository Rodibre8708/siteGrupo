...
function CreateLineupComponent() {
  ...
  // Efeito para carregar a escalação para edição do Back4app
  useEffect(() => {
    if (editId) {
      const fetchEscalacao = async () => {
        try {
          ...
          const escalacao = await query.get(editId);

          if (escalacao) {
            setLineupName(escalacao.get("name"));
            setFormation(escalacao.get("formation"));
            setLineupPositions(escalacao.get("players"));
            setSelectedLeague(escalacao.get("leagueId"));
            setSelectedTeam(escalacao.get("teamId"));
          }
        } catch (error) {
  ...
  const handleSaveLineup = async () => {
    ...
    if (editId) {
      escalacaoObject = await new Parse.Query(Escalacao).get(editId);
    } else {
      escalacaoObject = new Escalacao();
    }

    escalacaoObject.set("name", lineupName);
    escalacaoObject.set("formation", formation);
    escalacaoObject.set("players", lineupPositions);
    escalacaoObject.set("date", new Date().toLocaleDateString("pt-BR"));
    escalacaoObject.set("leagueId", selectedLeague);
    escalacaoObject.set("teamId", selectedTeam);

    try {
      const result = await escalacaoObject.save();
